...
public class VorbisFile{
  ...
  OggSyncState oy=new OggSyncState();
  ...
  OggStreamState os=new OggStreamState(); // take physical pages, weld into a logical
  // stream of packets
  VorbisDspState vd=new VorbisDspState(); // central working state for 
  // the packet->PCM decoder
  VorbisBlock vb=new VorbisBlock(vd); // local working space for packet->PCM decode
  ...
  private int get_next_page(OggPage page, long boundary){
    if(boundary>0)
      boundary+=offset;
    while(true){
      int more;
  ...
  private int get_prev_page(OggPage page) throws JOrbisException{
    long begin=offset; //!!!
    int ret;
    int offst=-1;
    while(offst==-1){
  ...
  int fetch_headers(VorbisInfo vi, VorbisComment vc, int[] serialno, OggPage og_ptr){
    OggPage og=new OggPage();
  ...
  void prefetch_all_headers(VorbisInfo first_i, VorbisComment first_c, int dataoffset)
      throws JOrbisException{
    OggPage og=new OggPage();
    int ret;

    vi=new VorbisInfo[links];
    vc=new VorbisComment[links];
  ...
  int open_seekable() throws JOrbisException{
    VorbisInfo initial_i=new VorbisInfo();
    VorbisComment initial_c=new VorbisComment();
    int serialno;
    long end;
    int ret;
    int dataoffset;
  ...
  public VorbisInfo getInfo(int link){
    if(seekable){
      if(link<0){
        if(decode_ready){
  ...
  public VorbisComment getComment(int link){
    if(seekable){
      if(link<0){
        if(decode_ready){
  ...
  public VorbisComment[] getComment(){
    return vc;
  }
